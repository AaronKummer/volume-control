...
void VolumeControlAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    float vol = *this->apvts.getRawParameterValue("VOLUME");
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        for (int sample = 0; sample < buffer.getNumSamples(); sample++) {
            channelData[sample] = buffer.getSample(channel, sample) * vol;
        }
...
juce::AudioProcessorValueTreeState::ParameterLayout VolumeControlAudioProcessor::GetParameterLayout()
{
    std::vector<std::unique_ptr<juce::RangedAudioParameter>> params;
    params.push_back(std::make_unique<juce::AudioParameterFloat>("VOLUME", "Volume", 0.0f, 10.0f, 5.0f));
